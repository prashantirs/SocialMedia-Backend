...
exports.myProfile=async(req,res)=>{
  try {
    //find user
    const user=await User.findById(req.user._id).populate("posts followers following"); //populate from schema of user find it
    res.status(200).json({
      success:true,
      user
    })
...
exports.getMyPosts=async(req,res)=>{
  try {
    //find user
    const user=await User.findById(req.user._id);//find my id logged in user
...
exports.getUserPost=async(req,res)=>{
  try {
    //find user
    const user=await User.findById(req.params.id);//find id on which we clicked
